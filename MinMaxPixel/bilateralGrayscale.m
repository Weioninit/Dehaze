function [ output ] = bilateralGrayscale( inputImg,sigma_s,sigma_r,window_size)
...
[row col d]=size(inputImg);

%check if the image is RGB (depth is more than 2)
% if d>2
% noisy_image=rgb2gray(inputImg);
% end
...
for i=1:r
    for j=1:c
        ...
        I=inputImg(imin:imax,jmin:jmax);
        
        range_filter=exp(-double(I-inputImg(i,j)).^2/(2*sigma_r^2)); % Range filter
