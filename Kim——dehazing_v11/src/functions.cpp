...
/*
Function: DownsampleImage
Description: Downsample the image to fixed sized image (640 x 480)
...
m_pnSmallYImg - output down sampled image
*/
void dehazing::DownsamplebigImage()
{
	int nX, nY;

	float fRatioY, fRatioX;
	// 다운샘플링 비율 결정
	fRatioX = (float)m_nWid / (float)640;
	fRatioY = (float)m_nHei / (float)480;
	...
	for (nY = 0; nY<480; nY++)
	{
		for (nX = 0; nX<640; nX++)
		{
			...
			m_pnSmallYImg[nY * 640 + nX] = m_pnYImg[(int)(nY*fRatioY)*m_nWid + (int)(nX*fRatioX)];
		}
	}
}
/* 
...
Parameters:(hidden)
m_pfSmallTransR - input transmission (640 x 480)
Return:
m_pfTransmission - output transmission

*/
void dehazing::UpsamplebigTransmission()
{
	int nX, nY;

	float fRatioY, fRatioX;
	// 업샘플링 비율 결정
	fRatioX = (float)640 / (float)m_nWid;
	fRatioY = (float)480 / (float)m_nHei;
	...
	for (nY = 0; nY<m_nHei; nY++)
	{
		...
		{
			// (1) 멤버 변수인 m_pfSmallTransR를 m_pfTransmission로 업샘플링
			m_pfTransmission[nY*m_nWid + nX] = m_pfSmallTrans[(int)(nY*fRatioY) * 640 + (int)(nX*fRatioX)];
		}
	}
}
/* 
