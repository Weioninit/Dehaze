...
*/
dehazing::dehazing(int nW, int nH, bool bPrevFlag, bool bPosFlag)  //构造函数的实现
{
	m_nWid			= nW;
	m_nHei			= nH;
...
*/
dehazing::dehazing(int nW, int nH, int nTBlockSize, bool bPrevFlag, bool bPosFlag, float fL1, float fL2, int nGBlock)//构造函数的实现
{
	m_nWid			= nW;
	m_nHei			= nH;
...
dehazing::~dehazing(void)//构造函数的实现
{
 ...
 */
void dehazing::AirlightEstimation(IplImage* imInput)//成员函数的实现
{
	...
	cvSetImageROI(imInput, cvRect(nWid/2+nWid%2, 0, nWid/2, nHei/2));
	cvCopyImage(imInput, iplUpperRight);
	cvSetImageROI(imInput, cvRect(0, nHei/2+nHei%2, nWid/2, nHei/2));
	cvCopyImage(imInput, iplLowerLeft);
	cvSetImageROI(imInput, cvRect(nWid/2+nWid%2, nHei/2+nHei%2, nWid/2, nHei/2));
...
void dehazing::HazeRemoval(IplImage* imInput, IplImage* imOutput, int nFrame)
{
	...
	if (nFrame == 0) //gaozw
	{
		...
		m_nAirlight = (((int)(uchar)m_anAirlight[0]*25 + (int)(uchar)m_anAirlight[1]*129 + (int)(uchar)m_anAirlight[2]*66 +128) >>8) + 16;//gaozw?

		cvReleaseImage(&imAir);
		cvResetImageROI(imInput);
	}
	
	IplImageToInt(imInput);//gaozw  RGB->YUV(Y)   input IplImage->m_pnYImg 
	// down sampling to fast estimation
	DownsampleImage();//m_pnYImg - input Y Image->m_pnSmallYImg - output down sampled image
	
	// trnasmission estimation
	TransmissionEstimation(m_pnSmallYImg,m_pfSmallTrans, m_pnSmallYImgP, m_pfSmallTransP, nFrame, 320, 240);
	//TransmissionEstimation(m_pnYImg, m_pfSmallTrans, m_pnSmallYImgP, m_pfSmallTransP, nFrame, m_nWid, m_nHei);
	...
	UpsampleTransmission();//gaozw 从320*240中上采样
	...
	cvWaitKey(-1);
	*/
	//GuidedFilterShiftableWindow(40.0f);
     FastGuidedFilter();
